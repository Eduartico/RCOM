...
int llopen(char* port) {
    ...
    do {
        ...
        printf("Time out. Attempts left: %d\n", protocol.numTransmissions);
        protocol.numTransmissions--;
        read_state = START;
        fd_write(fd, SET, sizeof(SET));
    } while (STOP == FALSE && protocol.numTransmissions > 0);
            
    if(STOP == TRUE)
        printf("Connection successful.\n");
    else
        printf("Connection failed. Retransmission attempts exceeded.\n");
